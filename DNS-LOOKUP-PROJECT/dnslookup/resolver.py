...
async def validate_dnssec(
    ...
    """Validate DNSSEC for an A record."""

    try:
        # Get A record + RRSIG(A)
        a_answer = await resolver.resolve(domain, "A")

        # Get DNSKEY
        key_answer = await resolver.resolve(domain, "DNSKEY")
        ...
        key_rrset = next(
            rrset
            for rrset in key_answer.response.answer
            if rrset.rdtype == dns.rdatatype.DNSKEY
        )

        # Validate the A record signature
        dns.dnssec.validate(
            a_rrset,
            rrsig_rrset,
